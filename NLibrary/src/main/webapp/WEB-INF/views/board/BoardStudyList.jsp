...
<body>
				...
				<table class="table table-condensed" id="board">
					...
					<tbody>
						<c:forEach var="board" items="${list}">
								<tr href="BoardDetail.nds?board_id=${board.board_id}&type=study" style="cursor:pointer;">
	...
	if(totalCount > 0) {
		...
		if(pageNumTemp > 1){
			%>
				<li><a href="BoardStudyList.nds?pageNum=1">«</a></li>
			<% 
		}
%><% 
		...
		if (isPrev) {
			...
			%>
				<li><a href="BoardStudyList.nds?pageNum="<%=goPrevPage%>">«</a></li>
			<% 
		} else {
		...
		for (int i = startPage; i <= endPage; i++) {
			if (i == pageNumTemp) {
				...
				%>
					<li><a href="BoardStudyList.nds?pageNum=<%=i%>"><%=i %></a></li>
				<% 
			}
		}
		if (isNext) {
			...
			%>
				 <li><a href="BoardStudyList.nds?ageNum=<%=goNextPage%>">»</a></li>
			<% 
		} else {
		...
		if(totalNumOfPage > pageNumTemp){
			%>
				<li><a href="BoardStudyList.nds?pageNum=<%=totalNumOfPage%>">»</a></li>
			<%
		}
	}
	%>

...
public class MainController {
	...
	public String searchResult(Model model, String key, String searchWord, String filter, String pageNum) {
		...
		for (Borrowing result : Result) {
			if (dao.getInfoBorrowingCount(result.getInformation_id()) != 0) {
				result.setBorrowing_state("대출가능");
			} else {
				result.setBorrowing_state("대출중");
			}
		}
	...
	public String SearchPage(Model model, String category_id, String filter, String pageNum) {
		...
		model.addAttribute("totalCount", bookInfoList.size());
		model.addAttribute("filter", filter);
		...
		model.addAttribute("indexCount", bookInfoList.size() - (pageNumTemp - 1) * 10);
	...
	public String getBorrowingList(Model model, String ISBN, HttpServletRequest request, String information_id,
			String category_id, String pageNum) {
